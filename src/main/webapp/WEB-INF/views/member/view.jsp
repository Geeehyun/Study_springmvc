...
<div class="container-fluid">
    ...
    <div class="container">
        <div class="card justify-content-md-center" style="width: 80%; margin: 0 auto";>
            <div class="card-header bg-primary text-white">내 정보 조회</div>
            <div class="card-body" style="padding: 30px">
                <h3 class="card-text text-primary text-center" id="div_result"></h3>
                <h3 class="card-text text-danger text-center" id="div_fail"></h3>
                <form action="/member/modify" method="post" id="frm" class="row g-3">
                    <div class="col-12">
                        <label for="user_id" class="form-label">아이디</label>
                        <div class="input-group mb-3 col-12">
                            <input type="text" class="form-control" aria-label="Recipient's username" aria-describedby="button-addon2"  id="user_id" name="user_id" maxlength="20"  value="${memberDTO['user_id']}"  readonly>
                        </div>
                    ...
                    <div class="col-12">
                        ...
                        <input type="text" class="form-control" id="addr1" name="addr1" maxlength="100" value="${memberDTO.addr1}" >
                    </div>
                    <div class="col-12">
                        <label for="addr2" class="form-label">주소2</label>
                        <input type="text" class="form-control" id="addr2" name="addr2" maxlength="100" value="${memberDTO.addr2}" >
                    </div>
                    <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                        <button class="btn btn-secondary" type="button" onclick="goLeave('${sessionScope["user_id"]}')">탈퇴</button>
    ...
    <script>
        ...
        let target = document.getElementById("div_result");
        target.innerText = "${result}";
        target.style.display = "block";
        </c:if>
        <c:if test="${!empty fail}">
        let target = document.getElementById("div_fail");
        target.innerText = "${fail}";
        target.style.display = "block";
        </c:if>

        function goLeave(user_id) {
            if(confirm("정말 탈퇴 하시겠습니까?")) {
                ...
                document.querySelector("#frm_leave").submit();
            }
        }
    </script>
